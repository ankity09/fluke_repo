...
df_audio = spark.sql(f"SELECT transcript_text as content, path as doc_uri FROM {UC_CATALOG_NAME}.{UC_SCHEMA_NAME}.{AUDIO_DATA_TABLE_NAME}")
df_video = spark.sql(f"SELECT transcript_text as content, path as doc_uri FROM {UC_CATALOG_NAME}.{UC_SCHEMA_NAME}.{VIDEO_DATA_TABLE_NAME}")
df = df_docs.union(df_audio)
docs = df.select("content", "doc_uri")
display(docs)


agent_description = """
The Agent is a RAG chatbot that answers questions about using Fluke Products. The Agent has access to a corpus of Fluke equipment manuals, and its task is to answer the user's questions by retrieving the relevant docs from the corpus and synthesizing a helpful, accurate response. The corpus covers a lot of info, but the Agent is specifically designed to interact with Support agents who have questions about Fluke equipment from their customers. So questions outside of this scope are considered irrelevant.
"""
question_guidelines = """
# User personas
...
# Example questions
- What is the use of *OPC?, *OPC, and *WAI on the 5730A?
